...
-- Telescope
nmap {
    '<M-BS>', '<cmd>lua require(\'telescope.builtin\').git_files()<cr>', silent
}
nmap {
    '<C-p>', '<cmd>lua require(\'telescope.builtin\').git_files()<cr>', silent
}
nmap {
...
-- Neogit mapping
nmap {
    '<C-g>', '<cmd>lua require("neogit").open({ kind = "split_above" })<cr>',
    silent
}
...
nmap {
    '<leader><leader>pc', ':PackerClean<CR>', {noremap = true, silent = false}
}
...
imap {
    '<C-l>', function() if ls.choice_active() then ls.change_choice(1) end end,
    silent
}
